--[[
    HubMenu.lua (MERGED: HubMenu UI + StealEggHub backend, WindUI stripped)
    LocalScript — Place in StarterGui
    Features & Fixes:
    - GLOBAL 2.5s Cooldown on ALL window controls (Close, Minimize, Maximize, Reopen, Keybind).
    - Anti-aliased smooth corners (8px/12px) preserved via automatic ClipsDescendants toggle.
    - Default background transparency set to 3% (0.03).
    - Player controls in the Home tab:
        * Custom WalkSpeed field & button
        * Custom JumpPower field & button
        * Infinite Jump toggle
        * Reset Stats button
        * Persistent settings on character respawn
]]
...
local ACTION_COOLDOWN = 2.5 -- Жесткий КД 2.5 секунды для ВСЕХ кнопок управления
...
-- ==================== COLOR SCHEME ====================
local COLORS = {
    TitleBar = Color3.fromRGB(20, 24, 20),
    Sidebar = Color3.fromRGB(18, 22, 18),
    Content = Color3.fromRGB(24, 28, 24),
    Hover = Color3.fromRGB(34, 40, 34),
    Selected = Color3.fromRGB(40, 48, 40),
    Text = Color3.fromRGB(235, 235, 235),
    SubText = Color3.fromRGB(150, 155, 150),
    Accent = Color3.fromRGB(120, 220, 90),
    Border = Color3.fromRGB(45, 50, 45),
    CardBg = Color3.fromRGB(30, 35, 30),
    InputBg = Color3.fromRGB(18, 22, 18),
}

local TWEEN_FAST = TweenInfo.new(0.18, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
local TWEEN_SMOOTH = TweenInfo.new(0.3, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out)
...
local function applyPlayerSettings()
    ...
    if hum then
        if playerSettings.lockWalkSpeed then
            hum.WalkSpeed = playerSettings.walkSpeed
        end

        if playerSettings.lockJumpPower then
...
-- Re-apply settings continuously to prevent game overrides
task.spawn(function()
    while true do
        task.wait(0.1)
...
mainFrame.BackgroundColor3 = COLORS.Content
mainFrame.BackgroundTransparency = 1
mainFrame.BorderSizePixel = 0
mainFrame.ClipsDescendants = false
mainFrame.Parent = screenGui
corner(8, mainFrame)
stroke(mainFrame, 1, 0.3)
...
titleBar.BackgroundTransparency = 0.03
...
titleBarFix.BackgroundTransparency = 0.03
...
titleSub.Position = UDim2.new(0, 105, 0, 0)
titleSub.Text = ("(@%s)"):format(player.Name)
titleSub.ZIndex = 4
titleSub.Parent = titleBar

local function createWindowButton(text, xOffset)
    ...
    btn.MouseEnter:Connect(function()
        TweenService:Create(btn, TWEEN_FAST, {TextColor3 = COLORS.Text}):Play()
    end)
    btn.MouseLeave:Connect(function()
        TweenService:Create(btn, TWEEN_FAST, {TextColor3 = COLORS.SubText}):Play()
    end)
    return btn
end

local closeBtn = createWindowButton("✕", -44)
local minimizeBtn = createWindowButton("—", -84)
local maximizeBtn = createWindowButton("▢", -124)
...
sidebarFix.BackgroundTransparency = 0.03
...
-- ==================== BACKGROUND TRANSPARENCY SYSTEM ====================
local backgroundPanels = { titleBar, titleBarFix, sidebar, sidebarFix, contentArea, contentFix }
local currentBackgroundTransparency = 0.03
local function setBackgroundTransparency(transparency)
    currentBackgroundTransparency = math.clamp(transparency, 0, 0.85)
    for _, panel in ipairs(backgroundPanels) do
...
-- ==================== HUB HELPERS (shared cards for merged tabs) ====================
local function mkScroll(page)
...
local function mkSection(parent, order, text)
    local lab = Instance.new("TextLabel")
    lab.Font = Enum.Font.GothamBold
    lab.TextSize = 13
    lab.TextColor3 = COLORS.Accent
    lab.TextXAlignment = Enum.TextXAlignment.Left
    lab.BackgroundTransparency = 1
    lab.Size = UDim2.new(1, -8, 0, 20)
...
local function mkToggle(parent, order, titleText, descText, initial, callback)
    ...
    frame.Size = UDim2.new(0, 48, 0, 24)
    frame.Position = UDim2.new(1, -64, 0.5, -12)
    frame.BackgroundColor3 = COLORS.TitleBar
    ...
    local function paint()
        ...
        TweenService:Create(frame, TWEEN_FAST, {
            BackgroundColor3 = state and COLORS.Accent or COLORS.TitleBar
        }):Play()
    end
    paint()
...
local function mkButton(parent, order, text, callback)
    local btn = Instance.new("TextButton")
    ...
    stroke(btn, 1, 0.4)
    btn.MouseEnter:Connect(function()
        TweenService:Create(btn, TWEEN_FAST, {BackgroundColor3 = COLORS.Hover}):Play()
    end)
    btn.MouseLeave:Connect(function()
        TweenService:Create(btn, TWEEN_FAST, {BackgroundColor3 = COLORS.Selected}):Play()
    end)
    btn.MouseButton1Click:Connect(function()
        if callback then
            task.spawn(callback)
...
local function mkStatus(parent, order, titleText, initialText)
    local card = Instance.new("Frame")
    card.Size = UDim2.new(1, -8, 0, 60)
    card.BackgroundColor3 = COLORS.CardBg
    card.BorderSizePixel = 0
    ...
    value.Size = UDim2.new(1, -24, 0, 18)
    value.Position = UDim2.new(0, 16, 0, 30)
...
local function selectTab(tabName)
    ...
    for name, btn in pairs(tabButtons) do
        ...
        TweenService:Create(btn, TWEEN_FAST, {
            BackgroundColor3 = selected and COLORS.Selected or COLORS.Sidebar,
        }):Play()
        TweenService:Create(btn.Indicator, TWEEN_FAST, {
            BackgroundTransparency = selected and 0 or 1
        }):Play()
        TweenService:Create(btn.Label, TWEEN_FAST, {
...
for i, tab in ipairs(TABS) do
    ...
    btn.MouseLeave:Connect(function()
        if currentTab ~= tab.name then
            TweenService:Create(btn, TWEEN_FAST, {BackgroundColor3 = COLORS.Sidebar}):Play()
        end
    end)
    btn.MouseButton1Click:Connect(function()
        selectTab(tab.name)
    ...
    if tab.name == "Home" then
        ...
        local function createInputCard(order, titleText, descText, defaultValue, callback)
            local card = Instance.new("Frame")
            card.Size = UDim2.new(1, -8, 0, 60)
            card.BackgroundColor3 = COLORS.CardBg
            card.BorderSizePixel = 0
            ...
            inputBox.Size = UDim2.new(0, 70, 0, 32)
            inputBox.Position = UDim2.new(1, -140, 0.5, -16)
            inputBox.BackgroundColor3 = COLORS.InputBg
            inputBox.Font = Enum.Font.GothamMedium
            inputBox.TextSize = 13
            inputBox.TextColor3 = COLORS.Accent
            inputBox.Text = tostring(defaultValue)
            inputBox.ClearTextOnFocus = false
            inputBox.Parent = card
            corner(6, inputBox)
            stroke(inputBox, 1, 0.3)

            local applyBtn = Instance.new("TextButton")
            applyBtn.Size = UDim2.new(0, 56, 0, 32)
            applyBtn.Position = UDim2.new(1, -64, 0.5, -16)
            applyBtn.BackgroundColor3 = COLORS.Selected
            ...
            local function applyValue()
                ...
                if num then
                    callback(num)
                    TweenService:Create(applyBtn, TWEEN_FAST, {BackgroundColor3 = COLORS.Accent}):Play()
                    task.wait(0.15)
                    TweenService:Create(applyBtn, TWEEN_FAST, {BackgroundColor3 = COLORS.Selected}):Play()
                else
                    inputBox.Text = tostring(defaultValue)
                end
            end
        ...
        createInputCard(1, "Set Ur WalkSpeed", "Set WalkSpeed continuously every 0.1s (Default: 16)", playerSettings.walkSpeed, function(val)
            playerSettings.walkSpeed = val
            playerSettings.lockWalkSpeed = true
            applyPlayerSettings()
        end)

        createInputCard(2, "Set Ur JumpPower", "Set JumpPower continuously every 0.1s (Default: 50)", playerSettings.jumpPower, function(val)
            playerSettings.jumpPower = val
            playerSettings.lockJumpPower = true
            applyPlayerSettings()
        end)

        local infJumpCard = Instance.new("Frame")
        infJumpCard.Size = UDim2.new(1, -8, 0, 60)
        infJumpCard.BackgroundColor3 = COLORS.CardBg
        infJumpCard.BorderSizePixel = 0
        infJumpCard.LayoutOrder = 3
        infJumpCard.Parent = scroll
        corner(8, infJumpCard)
        stroke(infJumpCard, 1, 0.4)

        local infTitle = Instance.new("TextLabel")
        infTitle.Font = Enum.Font.GothamBold
        infTitle.TextSize = 14
        infTitle.TextColor3 = COLORS.Text
        infTitle.TextXAlignment = Enum.TextXAlignment.Left
        infTitle.BackgroundTransparency = 1
        infTitle.Size = UDim2.new(1, -80, 0, 20)
        infTitle.Position = UDim2.new(0, 16, 0, 10)
        infTitle.Text = "Infinite Jump"
        infTitle.Parent = infJumpCard

        local infSub = Instance.new("TextLabel")
        infSub.Font = Enum.Font.Gotham
        infSub.TextSize = 12
        infSub.TextColor3 = COLORS.SubText
        infSub.TextXAlignment = Enum.TextXAlignment.Left
        infSub.BackgroundTransparency = 1
        infSub.Size = UDim2.new(1, -80, 0, 18)
        infSub.Position = UDim2.new(0, 16, 0, 30)
        infSub.Text = "Allows jumping indefinitely in mid-air"
        infSub.Parent = infJumpCard

        local toggleFrame = Instance.new("Frame")
        toggleFrame.Size = UDim2.new(0, 48, 0, 24)
        toggleFrame.Position = UDim2.new(1, -64, 0.5, -12)
        toggleFrame.BackgroundColor3 = COLORS.TitleBar
        toggleFrame.BorderSizePixel = 0
        toggleFrame.Parent = infJumpCard
        corner(12, toggleFrame)
        stroke(toggleFrame, 1, 0.3)
        ...
        toggleBtn.Text = ""
        toggleBtn.Parent = infJumpCard
        toggleBtn.MouseButton1Click:Connect(function()
            playerSettings.infiniteJump = not playerSettings.infiniteJump
            local targetDotPos = playerSettings.infiniteJump and UDim2.new(1, -21, 0.5, -9) or UDim2.new(0, 3, 0.5, -9)
            local targetBgColor = playerSettings.infiniteJump and COLORS.Accent or COLORS.TitleBar
            TweenService:Create(toggleDot, TWEEN_FAST, {Position = targetDotPos}):Play()
            TweenService:Create(toggleFrame, TWEEN_FAST, {BackgroundColor3 = targetBgColor}):Play()
        end)

        local resetBtn = Instance.new("TextButton")
        resetBtn.Size = UDim2.new(1, -8, 0, 38)
        resetBtn.BackgroundColor3 = COLORS.Selected
        resetBtn.Font = Enum.Font.GothamBold
        resetBtn.TextSize = 13
        resetBtn.TextColor3 = COLORS.Text
        resetBtn.Text = "↻ Reset Speed & Jump to Default"
        resetBtn.LayoutOrder = 4
        resetBtn.Parent = scroll
        corner(8, resetBtn)
        stroke(resetBtn, 1, 0.4)
        resetBtn.MouseButton1Click:Connect(function()
            playerSettings.walkSpeed = 16
            playerSettings.jumpPower = 50
            playerSettings.lockWalkSpeed = false
            playerSettings.lockJumpPower = false
    ...
    elseif tab.name == "Config" then
        ...
        settingCard.Name = "TransparencySetting"
        settingCard.Size = UDim2.new(1, 0, 0, 60)
        settingCard.Position = UDim2.new(0, 0, 0, 42)
        settingCard.BackgroundColor3 = COLORS.CardBg
        settingCard.BorderSizePixel = 0
        settingCard.Parent = page
        corner(8, settingCard)
        stroke(settingCard, 1, 0.4)
        ...
        cardTitle.Size = UDim2.new(1, -80, 0, 20)
        cardTitle.Position = UDim2.new(0, 16, 0, 10)
        cardTitle.Text = "Background Transparency"
        cardTitle.Parent = settingCard

        local cardSub = Instance.new("TextLabel")
        cardSub.Font = Enum.Font.Gotham
        cardSub.TextSize = 12
        cardSub.TextColor3 = COLORS.SubText
        cardSub.TextXAlignment = Enum.TextXAlignment.Left
        cardSub.BackgroundTransparency = 1
        cardSub.Size = UDim2.new(1, -80, 0, 18)
        cardSub.Position = UDim2.new(0, 16, 0, 30)
        cardSub.Text = "Enable subtle 3% background transparency"
        cardSub.Parent = settingCard
        ...
        toggleFrame.BorderSizePixel = 0
        toggleFrame.Parent = settingCard
        corner(12, toggleFrame)
        stroke(toggleFrame, 1, 0.3)

        local toggleDot = Instance.new("Frame")
        toggleDot.Size = UDim2.new(0, 18, 0, 18)
        toggleDot.Position = UDim2.new(1, -21, 0.5, -9)
        toggleDot.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
        toggleDot.BorderSizePixel = 0
        toggleDot.Parent = toggleFrame
        corner(9, toggleDot)
        ...
        toggleBtn.MouseButton1Click:Connect(function()
            ...
            local targetDotPos = isTransparent and UDim2.new(1, -21, 0.5, -9) or UDim2.new(0, 3, 0.5, -9)
            local targetBgColor = isTransparent and COLORS.Accent or COLORS.TitleBar
            TweenService:Create(toggleDot, TWEEN_FAST, {Position = targetDotPos}):Play()
            TweenService:Create(toggleFrame, TWEEN_FAST, {BackgroundColor3 = targetBgColor}):Play()
            setBackgroundTransparency(isTransparent and 0.03 or 0)
        end)

        -- ===== StealEggHub: system (merged) =====
        local sysY = 110
        local function sysButton(text, callback)
            local btn = Instance.new("TextButton")
            btn.Size = UDim2.new(1, 0, 0, 38)
            btn.Position = UDim2.new(0, 0, 0, sysY)
            btn.BackgroundColor3 = COLORS.Selected
            btn.Font = Enum.Font.GothamBold
            btn.TextSize = 13
            btn.TextColor3 = COLORS.Text
    ...
    elseif tab.name == "Egg" then
        --==================== EGG FARM CONTROLLER (from Dice Hub, merged) ====================
        local RStorage = game:GetService("ReplicatedStorage")
        local eggState = { enabled = false }

        local function findRemote(name)
            local ok, res = pcall(function()
                return RStorage.Packages.Networking[name]
            end)
            return ok and res or nil
        end
        local AskCarry = findRemote("RF/EggWorld/AskFieldEggCarry")

        local function isEgg(inst)
            if not inst:IsA("Tool") then return false end
            local name = string.lower(inst.Name)
            if string.find(name, "sword") or string.find(name, "radar")
                or string.find(name, "basket") or string.find(name, "punch") then
                return false
            end
            return inst:GetAttribute("EggUid") or inst:GetAttribute("UID")
                or inst:GetAttribute("Category") or inst:GetAttribute("ItemType") == "Egg"
        ...
        local function firePromptsNear(pos)
            local uid = nil
        ...
        local function tryCarryBackup(uid)
            if not AskCarry then return end
            task.spawn(function()
                pcall(function()
                    if AskCarry:IsA("RemoteFunction") then
                        AskCarry:InvokeServer({ ["Uid"] = uid })
                    else
                        AskCarry:FireServer({ ["Uid"] = uid })
                    end
                end)
            end)
        end

        -- instant completions of any hold prompt while enabled
        task.spawn(function()
            game:GetService("ProximityPromptService").PromptButtonHoldBegan:Connect(function(p)
                if eggState.enabled and typeof(fireproximityprompt) == "function" then
                    fireproximityprompt(p)
                end
            end)
        end)

        -- E => fire prompts + carry-invoke backup
        UserInputService.InputBegan:Connect(function(input, gameProcessed)
            if gameProcessed then return end
            if input.KeyCode ~= Enum.KeyCode.E then return end
            if not eggState.enabled then return end
            local char = player.Character
            local hrp = char and char:FindFirstChild("HumanoidRootPart")
            if not hrp then return end
            local uid = firePromptsNear(hrp.Position)
            if uid then
                task.wait(0.05)
                tryCarryBackup(uid)
            end
        ...
        toggleCard.Size = UDim2.new(1, -8, 0, 60)
        toggleCard.BackgroundColor3 = COLORS.CardBg
        toggleCard.BorderSizePixel = 0
        toggleCard.Parent = scroll
        corner(8, toggleCard)
        stroke(toggleCard, 1, 0.4)

        local togTitle = Instance.new("TextLabel")
        togTitle.Size = UDim2.new(1, -80, 0, 20)
        togTitle.Position = UDim2.new(0, 16, 0, 10)
        togTitle.BackgroundTransparency = 1
        togTitle.Font = Enum.Font.GothamBold
        togTitle.TextSize = 14
        togTitle.TextColor3 = COLORS.Text
        togTitle.TextXAlignment = Enum.TextXAlignment.Left
        togTitle.Text = "Instant Egg Pickup"
        togTitle.Parent = toggleCard

        local togSub = Instance.new("TextLabel")
        togSub.Size = UDim2.new(1, -80, 0, 18)
        togSub.Position = UDim2.new(0, 16, 0, 30)
        togSub.BackgroundTransparency = 1
        togSub.Font = Enum.Font.Gotham
        togSub.TextSize = 12
        togSub.TextColor3 = COLORS.SubText
        togSub.TextXAlignment = Enum.TextXAlignment.Left
        togSub.Text = "Press E near an egg -> instant pickup"
        togSub.Parent = toggleCard

        local togFrame = Instance.new("Frame")
        togFrame.Size = UDim2.new(0, 48, 0, 24)
        togFrame.Position = UDim2.new(1, -64, 0.5, -12)
        togFrame.BackgroundColor3 = COLORS.TitleBar
        togFrame.BorderSizePixel = 0
        togFrame.Parent = toggleCard
        corner(12, togFrame)
        stroke(togFrame, 1, 0.3)

        local togDot = Instance.new("Frame")
        togDot.Size = UDim2.new(0, 18, 0, 18)
        togDot.Position = UDim2.new(0, 3, 0.5, -9)
        ...
        togBtn.Text = ""
        togBtn.Parent = toggleCard
        togBtn.MouseButton1Click:Connect(function()
            eggState.enabled = not eggState.enabled
            local targetDotPos = eggState.enabled and UDim2.new(1, -21, 0.5, -9) or UDim2.new(0, 3, 0.5, -9)
            local targetBgColor = eggState.enabled and COLORS.Accent or COLORS.TitleBar
            TweenService:Create(togDot, TWEEN_FAST, {Position = targetDotPos}):Play()
            TweenService:Create(togFrame, TWEEN_FAST, {BackgroundColor3 = targetBgColor}):Play()
        end)

        local statusCard = Instance.new("Frame")
        statusCard.Size = UDim2.new(1, -8, 0, 60)
        statusCard.BackgroundColor3 = COLORS.CardBg
        statusCard.BorderSizePixel = 0
        statusCard.Parent = scroll
        corner(8, statusCard)
        stroke(statusCard, 1, 0.4)

        local statusTitle = Instance.new("TextLabel")
        statusTitle.Size = UDim2.new(1, -24, 0, 20)
        statusTitle.Position = UDim2.new(0, 16, 0, 10)
        statusTitle.BackgroundTransparency = 1
        statusTitle.Font = Enum.Font.GothamBold
        statusTitle.TextSize = 14
        statusTitle.TextColor3 = COLORS.Text
        statusTitle.TextXAlignment = Enum.TextXAlignment.Left
        statusTitle.Text = "Egg Status"
        statusTitle.Parent = statusCard

        local statusText = Instance.new("TextLabel")
        statusText.Size = UDim2.new(1, -24, 0, 18)
        statusText.Position = UDim2.new(0, 16, 0, 30)
        statusText.BackgroundTransparency = 1
        statusText.Font = Enum.Font.Gotham
        statusText.TextSize = 12
        statusText.TextColor3 = COLORS.SubText
        statusText.TextXAlignment = Enum.TextXAlignment.Left
        statusText.Text = "Carried: 0 | Mode: OFF"
        statusText.Parent = statusCard

        task.spawn(function()
            while true do
                task.wait(0.5)
                statusText.Text = "Carried: " .. countCarried()
                    .. " | Mode: " .. (eggState.enabled and "ON" or "OFF")
            end
        end)

        -- ===== StealEggHub: target selection (merged) =====
        mkSection(scroll, 10, "TARGET ZONES")
        local zoneGrid = Instance.new("Frame")
        zoneGrid.Size = UDim2.new(1, -8, 0, 210)
        zoneGrid.BackgroundTransparency = 1
        zoneGrid.LayoutOrder = 11
        zoneGrid.Parent = scroll
        local zoneLayout = Instance.new("UIGridLayout")
        zoneLayout.CellSize = UDim2.new(0.48, 0, 0, 30)
        ...
        for zoneIdx, zone in ipairs(Steal.zones) do
            ...
            local function paintZone()
                if Steal.h.selectedZones and Steal.h.selectedZones[zone] then
                    ...
                    zoneBtn.BackgroundColor3 = COLORS.CardBg
                    zoneBtn.TextColor3 = COLORS.SubText
                    zoneBtn.Text = zone
                end
            end
            paintZone()
        ...
        rarityGrid.Size = UDim2.new(1, -8, 0, 174)
        rarityGrid.BackgroundTransparency = 1
        rarityGrid.LayoutOrder = 13
        rarityGrid.Parent = scroll
        local rarityLayout = Instance.new("UIGridLayout")
        rarityLayout.CellSize = UDim2.new(0.48, 0, 0, 30)
        ...
        for rarityIdx, rarity in ipairs(Steal.rarities) do
            ...
            rarityBtn.LayoutOrder = rarityIdx
            ...
            local function paintRarity()
                if Steal.h.selectedRarities and Steal.h.selectedRarities[rarity] then
                    ...
                    rarityBtn.BackgroundColor3 = COLORS.CardBg
                    rarityBtn.TextColor3 = COLORS.SubText
                    rarityBtn.Text = rarity
                end
            end
            paintRarity()
    ...
    elseif tab.name == "Stats" then
        ...
        mkSection(statsScroll, 1, "LIVE DASHBOARD")
        local setDash = mkStatus(statsScroll, 2, "Status", "Ready")
        task.spawn(function()
            while screenGui.Parent do
                task.wait(0.5)
                local mode = "Idle"
        ...
        placeholder.Position = UDim2.new(0, 0, 0, 42)
        placeholder.Text = "Add your content for the \"" .. tab.name .. "\" tab here."
...
profileFrame.Size = UDim2.new(1, -16, 0, 60)
profileFrame.Position = UDim2.new(0, 8, 1, -70)
profileFrame.BackgroundColor3 = COLORS.Hover
profileFrame.BorderSizePixel = 0
profileFrame.Parent = sidebar
corner(8, profileFrame)

local avatarOk, avatarContent = pcall(function()
...
avatar.BackgroundColor3 = COLORS.Selected
if avatarOk then
    avatar.Image = avatarContent
end
...
idLabel.TextSize = 11
idLabel.TextColor3 = COLORS.SubText
idLabel.TextXAlignment = Enum.TextXAlignment.Left
idLabel.BackgroundTransparency = 1
idLabel.Size = UDim2.new(1, -60, 0, 16)
idLabel.Position = UDim2.new(0, 58, 0, 30)
idLabel.Text = "@" .. player.Name
idLabel.Parent = profileFrame

-- ==================== MINIMIZE / MAXIMIZE (WITH 2.5s COOLDOWN) ====================
...
minimizeBtn.MouseButton1Click:Connect(function()
    ...
    local targetSize = minimized and UDim2.new(expandedSize.X.Scale, expandedSize.X.Offset, 0, 46) or expandedSize
    local tween = TweenService:Create(mainFrame, TWEEN_SMOOTH, {Size = targetSize})
    tween:Play()
...
local normalSize, normalPos = mainFrame.Size, mainFrame.Position
...
maximizeBtn.MouseButton1Click:Connect(function()
    ...
    if isMaximized then
        normalSize, normalPos = mainFrame.Size, mainFrame.Position
        TweenService:Create(mainFrame, TWEEN_SMOOTH, {
            Size = UDim2.new(0, 960, 0, 600),
...
-- ==================== OPEN / CLOSE CONTROLLER (WITH 2.5s COOLDOWN) ====================
local menuOpen = true
...
local function toggleMenu()
    ...
    if menuOpen then
        ...
        targetPos = UDim2.new(0.5, -expandedSize.X.Offset / 2, 0.5, -expandedSize.Y.Offset / 2)
        activeTween = TweenService:Create(mainFrame, TweenInfo.new(0.3, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
            Size = targetSize,
            Position = targetPos
        })
    else
        ...
        activeTween = TweenService:Create(mainFrame, TweenInfo.new(0.25, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {
            Size = targetSize,
            Position = targetPos
        })
    end
...
reopenButton.BackgroundColor3 = COLORS.TitleBar
reopenButton.Text = "☰"
...
corner(23, reopenButton)
stroke(reopenButton, 1, 0.3)

reopenButton.MouseButton1Click:Connect(function()
    if not menuOpen and canClick() then
        toggleMenu()
    end
end)
...
task.spawn(function()
    while true do
        task.wait(0.05)
        if not menuOpen and not mainFrame.Visible then
